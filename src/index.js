...
'<block type="move_right" id="move_right" x="0" y="40"></block>' +
'<block type="move_up" id="move_up" x="0" y="80"></block>' +
...
function moveRight() {
    ...
    if (current_state.y < COLS - 1) {
        ...
        if (current_state.x === TV_STATE.x && current_state.y === TV_STATE.y) {
            inErrorState=true;
            grid.classList.add("error");
            alert("You got distracted by the TV! Please try again");
        }
    } else {
        inErrorState = true;
        grid.classList.add("error");
